...
public class MenuRankingDetail extends AppCompatActivity implements View.OnClickListener {
    ...
    String temperature, cup, unitPrice, size, img, price, getType, menuPrice, soprice;
    int totalPrice;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        menuPrice = detailIntent.getStringExtra("menuPrice");
        menuPriceDetail.setText(menuPrice);
        ...
        numberPicker.setValueChangedListener(new ValueChangedListener() {
            //String strPrice = menuPriceDetail.getText().toString();
            //String subPrice = strPrice.substring(0, strPrice.length()-1).replace(",",""); //마지막 문자("원")와 컴마 제거.
            @Override
            public void valueChanged(int value, ActionEnum action) {
                Log.i("strPrice", menuPrice);
                int menuQuantity = numberPicker.getValue(); //메뉴 수량
                int mPrice = Integer.parseInt(menuPrice);
                DecimalFormat format = new DecimalFormat("#,000"); //형식 바꿈.
                totalPrice = menuQuantity * mPrice;
        ...
        try {
            String obj = new MenuRankingDetail.RankingDetailJsonTask().execute(menuNum).get();
            Log.i("json", ""+obj);
            parseJson(obj);
        } catch (ExecutionException e) {
            e.printStackTrace();
...
*/
    ...
    private void parseJson(String json) throws JSONException {
        ...
        menuNameDetail.setText(name);
        DecimalFormat format = new DecimalFormat("#,000"); //형식 바꿈.
        int mnprice=Integer.parseInt(price);
        String addPrice =  format.format(mnprice) + "원";
        menuPriceDetail.setText(addPrice);
        Picasso.with(this).load("http://cafein.freehost.kr/uploadImg/"+img).into(menuImgDetail);


    }


    class RankingDetailJsonTask extends AsyncTask<String, String, String> {
        String sendMsg;
        String receiveMsg;
        ...
        protected String doInBackground(String... strings) {
            try {
                String str;
                URL url = new URL("http://cafein.freehost.kr/coffeeRankingDetailJSON.jsp");
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
                conn.setRequestMethod("POST");
                OutputStreamWriter osw = new OutputStreamWriter(conn.getOutputStream());
