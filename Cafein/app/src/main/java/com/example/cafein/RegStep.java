...
public class RegStep extends AppCompatActivity implements CompoundButton.OnCheckedChangeListener, View.OnClickListener {
    ...
    Boolean allChk, chk1, chk2, chk3 = false; //check 상태.
    Button step1NextBtn, step2NextBtn, stepEndNextBtn, diaAgreeBtn1, diaAgreeBtn2, diaAgreeBtn3;
    TextInputLayout nameEdit, birthEdit, regIdEdit, regPwEdit, regPwConfirmEdit;
    ImageView stepClose, popUp1Close, popUp2Close, popUp3Close;
    RelativeLayout step1, step2, stepEnd, chkBox1PopUp, chkBox2PopUp, chkBox3PopUp;
    ...
    public void onClick(View v){
        switch (v.getId()){
            ...
            case R.id.step2NextBtn:
                ...
                String usBirth = birthEdit.getEditText().getText().toString().trim(); //생년월일
                ...
                Log.i("뭐가 문제지?","형식&빈칸: " + validateVal + "비밀번호 일치: " + equalPw);


                if (validateVal && equalPw) {
                    try {
                        String result  = new RegisterTask().execute(usID, usPw, usName, usBirth).get();
                        //Log.i("성공", result.substring(0,2)+"성공?");
    ...
    class RegisterTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) {
            try {
                String str;
                URL url = new URL("http://cafein.freehost.kr/insertUser.jsp");
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
                conn.setRequestMethod("POST");
                OutputStreamWriter osw = new OutputStreamWriter(conn.getOutputStream());
                sendMsg = "usID="+strings[0]+"&usPw="+strings[1]+"&usName="+strings[2]+"&usBirth="+strings[3];
                osw.write(sendMsg);
                osw.flush();
